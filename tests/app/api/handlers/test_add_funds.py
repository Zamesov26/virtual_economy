...
class TestAddFunds:
    ...
    async def test_success(self, client: AsyncClient, session_maker, override_redis: Redis):
    ...
    async def test_unprocessable_entity_missing_idempotency_key(self, client: AsyncClient, session_maker, override_redis):
        async with session_maker() as session:
    ...
    async def test_unprocessable_entity_when_amount_negative(self, client: AsyncClient, session_maker, override_redis):
        async with session_maker() as session:
            user = await self._create_user(session)
    ...
    async def test_unprocessable_entity_when_amount_exceeds_limit(self, client: AsyncClient, session_maker, override_redis):
        async with session_maker() as session:
            user = await self._create_user(session)
    ...
    async def test_conflict_when_idempotency_duplicate(self, client: AsyncClient, session_maker, override_redis):
        async with session_maker() as session:
    ...
    async def test_not_found_when_user_not_exists(
        self, client: AsyncClient, override_redis: Redis
    ):
        response = await client.post(
